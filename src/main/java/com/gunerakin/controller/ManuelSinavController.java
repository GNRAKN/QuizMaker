...
public class ManuelSinavController {
	...
	public void sessionRemoveAttribute(HttpSession session) {
	...
	public Sinav sessionGetAttribute(HttpSession session) {
	...
	public ModelAndView sinavOlustur(@ModelAttribute Sinav sinav, HttpSession session) {
		...
		sessionRemoveAttribute(session);

		HashSet<Kategori> kategoriler = new HashSet<Kategori>();

		for (Kategori kategori : soruService.listeleKategoriBySoru()) { // veritabaninda olan sorularin kategorileri
																		// view'e gonderiliyor ki sinav olustururken
																		// soru bulunmayan kategori secilemesin.
	...
	public String soruKaldir(HttpSession session, @RequestParam("soru_id") long soru_id) { 
		Sinav sinav = sessionGetAttribute(session);
